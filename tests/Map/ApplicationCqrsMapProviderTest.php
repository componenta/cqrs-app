...
final readonly class ApplicationMapTestBaseProvider implements CqrsMapProviderInterface
{
    ...
    public function map(): CqrsMap { return $this->map; }
}

final class ApplicationMapTestContainer implements ContainerInterface
{
    /** @var array<string, int> */ public array $gets = [];
    /** @param array<string, mixed> $entries */
    public function __construct(private readonly array $entries) {}
    public function get(string $id): mixed { $this->gets[$id] = ($this->gets[$id] ?? 0) + 1; return $this->entries[$id] ?? throw new RuntimeException($id); }
...
it('enables live discovery only for the development environment', function (?string $environment): void {
    $config = new Config([], $environment === null ? null : new Environment(['APP_ENV' => $environment]));
    ...
    expect($provider->map()->commandHandler(ApplicationMapTestCommand::class)?->service)->toBe(ApplicationMapTestHandler::class);
})->with([
    'missing environment defaults to development' => [null],
    'development' => ['development'],
]);

it('does not resolve discovery services outside development', function (string $environment): void {
    $container = new ApplicationMapTestContainer([
...
it('allows disabling development discovery explicitly', function (): void {
    $container = new ApplicationMapTestContainer([
        ConfigKey::CONFIG => new Config([AppConfigKey::DISCOVERY_ENABLED => false]),
        CqrsMapProviderInterface::class => new ApplicationMapTestBaseProvider(),
    ]);
    $provider = (new ApplicationCqrsMapProviderFactory())($container);
...
it('rejects a non-bool discovery flag', function (): void {
    $container = new ApplicationMapTestContainer([
        ConfigKey::CONFIG => new Config([AppConfigKey::DISCOVERY_ENABLED => 'yes']),
        CqrsMapProviderInterface::class => new ApplicationMapTestBaseProvider(),
    ]);
    expect(fn () => (new ApplicationCqrsMapProviderFactory())($container))->toThrow(InvalidArgumentException::class, 'must be bool');
});

it('validates and deduplicates configured generic metadata attributes', function (): void {
    $container = new ApplicationMapTestContainer([
        ConfigKey::CONFIG => new Config([
            ConfigKey::COMMAND_METADATA_ATTRIBUTES => [ApplicationMapTestMetadata::class, ApplicationMapTestMetadata::class],
        ]),
    ]);
    ...
    expect($index->map()->toArray()['commands']['metadata'])->toBe([
        ApplicationMapTestMetadataCommand::class => [ApplicationMapTestMetadata::class => ['arguments' => [0 => 'discovered']]],
    ]);
});

it('rejects configured metadata classes that are not PHP attributes', function (): void {
    $container = new ApplicationMapTestContainer([
        ConfigKey::CONFIG => new Config([ConfigKey::COMMAND_METADATA_ATTRIBUTES => [stdClass::class]]),
    ]);
    expect(fn () => (new CqrsDiscoveryIndexFactory())($container))->toThrow(InvalidArgumentException::class, 'not declared with #[Attribute]');
});
